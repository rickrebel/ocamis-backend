...
def hydrateCol(row, all_headers):
    ...
    if not len(cols):
        return False
    for lis in range(len(row)): 
        for idx_head, header in enumerate(all_headers):
            try:
                hydrated[header] = cols[lis][idx_head]
                dtaclues.append(hydrated)
            except Exception as e:
                print(cols)
                print(hydrated)
                return False
 ...
 for lis in range(len(listval)):
     for key in range(len(listval[lis])):
             print(listval[lis][key])
...
hydrated= {}
for lis in range(len(listval)):
    for idx_head, header in listval[lis]:
            hydrated[header] = col[lis][idx_head]
        try:
