...
create table if not exists public.catalog_typology
(
    ...
    aggregate_to_id   integer
        constraint catalog_typology_aggregate_to_id_35cf4cf2_fk_catalog_t
            references public.catalog_typology
            deferrable initially deferred,
    is_aggregate      boolean
);

alter table public.catalog_typology
    owner to user_desabasto;

create index if not exists catalog_typology_aggregate_to_id_35cf4cf2
    on public.catalog_typology (aggregate_to_id);
...
create table if not exists public.desabasto_institution
(
    id          serial
        primary key,
    name        varchar(255) not null,
...
alter table public.desabasto_institution
    owner to user_desabasto;
...
create table if not exists public.catalog_jurisdiction
(
    id             integer generated by default as identity
        primary key,
    name           varchar(255) not null,
    ...
    institution_id integer      not null
        constraint catalog_jurisdiction_institution_id_4f9ecb3e_fk_desabasto
            references public.desabasto_institution
            deferrable initially deferred,
...
alter table public.catalog_jurisdiction
    owner to user_desabasto;

create index if not exists catalog_jurisdiction_institution_id_4f9ecb3e
    on public.catalog_jurisdiction (institution_id);

create index if not exists catalog_jurisdiction_state_id_3ed5b9ad
    on public.catalog_jurisdiction (state_id);

create table if not exists public.catalog_municipality
(
    id         serial
        primary key,
...
alter table public.catalog_municipality
    owner to user_desabasto;

create index if not exists catalog_municipality_state_id_8f05caae
    on public.catalog_municipality (state_id);
...
create table if not exists public.catalog_delegation
(
    id               serial
        primary key,
    name             varchar(255) not null,
...
alter table public.catalog_delegation
    owner to user_desabasto;

create index if not exists catalog_delegation_entity_id_f3eabeb3
    on public.catalog_delegation (entity_id);

create index if not exists catalog_delegation_institution_id_875b3b00
    on public.catalog_delegation (institution_id);

create index if not exists catalog_delegation_state_id_735f63d1
    on public.catalog_delegation (state_id);

create table if not exists public.desabasto_clues
(
    id                      serial
        primary key,
    state_id                integer      not null
...
alter table public.desabasto_clues
    owner to user_desabasto;
...
(
    ...
    clues_id               integer
        constraint catalog_entity_clues_id_b6a62f84_fk_desabasto_clues_id
            references public.desabasto_clues
            deferrable initially deferred,
    institution_id         integer                                                    not null
...
(
    ...
    clues_id         integer
        constraint med_cat_medicalunity_clues_id_11cefd9d_fk_desabasto_clues_id
            references public.desabasto_clues
            deferrable initially deferred,
    delegation_id    integer
