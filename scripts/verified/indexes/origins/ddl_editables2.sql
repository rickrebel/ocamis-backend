...
create table if not exists fm_1_202109_drug
(
    uuid              uuid    not null,
...
alter table fm_1_202109_drug
    owner to user_yeeko;

create table if not exists fm_1_202109_missingfield
(
    uuid           uuid                     not null,
    original_value text,
    final_value    text,
...
alter table fm_1_202109_missingfield
    owner to user_yeeko;

create table if not exists fm_1_202109_missingrow
(
    uuid          uuid    not null,
    original_data jsonb,
    drug_id       uuid,
...
alter table fm_1_202109_missingrow
    owner to user_yeeko;

create table if not exists fm_1_202109_rx
(
...
alter table fm_1_202109_rx
    owner to user_yeeko;

create table if not exists fm_1_202110_drug
(
    uuid              uuid    not null,
    prescribed_amount smallint,
    delivered_amount  smallint,
...
alter table fm_1_202110_drug
    owner to user_yeeko;

create table if not exists fm_1_202110_missingfield
(
    uuid           uuid                     not null,
    original_value text,
    final_value    text,
...
alter table fm_1_202110_missingfield
    owner to user_yeeko;

create table if not exists fm_1_202110_missingrow
(
    uuid          uuid    not null,
    original_data jsonb,
    drug_id       uuid,
...
alter table fm_1_202110_missingrow
    owner to user_yeeko;

create table if not exists fm_1_202110_rx
(
    uuid_folio         uuid        not null,
    folio_ocamis       varchar(64) not null,
    folio_document     varchar(46) not null,
...
(
    ...
    constraint inai_entityweek_entity_id_year_week_iso__285ba5db_uniq
        unique (entity_id, year_week, iso_delegation_id, year_month)
);
...
create table if not exists inai_step
(
    id              integer generated by default as identity
        primary key,
    error_process   jsonb,
    entity_month_id integer     not null
        constraint inai_step_entity_month_id_bf0db1f5_fk_inai_entitymonth_id
            references inai_entitymonth
            deferrable initially deferred,
    stage_id        varchar(80) not null
        constraint inai_step_stage_id_2b97d912_fk_classify_task_stage_name
            references classify_task_stage
            deferrable initially deferred,
    status_id       varchar(80) not null
        constraint inai_step_status_id_488c3862_fk_classify_task_statustask_name
            references classify_task_statustask
            deferrable initially deferred
);

alter table inai_step
    owner to user_yeeko;

create index if not exists inai_step_entity_month_id_bf0db1f5
    on inai_step (entity_month_id);

create index if not exists inai_step_stage_id_2b97d912
    on inai_step (stage_id);

create index if not exists inai_step_stage_id_2b97d912_like
    on inai_step (stage_id varchar_pattern_ops);

create index if not exists inai_step_status_id_488c3862
    on inai_step (status_id);

create index if not exists inai_step_status_id_488c3862_like
    on inai_step (status_id varchar_pattern_ops);
...
(
    ...
    document_type      varchar(50),
    entity_id          integer     not null
        constraint formula_rx_entity_id_d8660a11_fk_geo_entity_id
            references geo_entity
            deferrable initially deferred,
    area_id            varchar(32),
    delivered_final_id varchar(32)
...
create table if not exists fm_53_202301_rx
(
    uuid_folio         uuid        not null
        primary key,
    folio_ocamis       varchar(64) not null,
    folio_document     varchar(46) not null,
    iso_year           smallint    not null
        constraint fm_53_202301_rx_iso_year_check
            check (iso_year >= 0),
    iso_week           smallint    not null
        constraint fm_53_202301_rx_iso_week_check
            check (iso_week >= 0),
    iso_day            smallint
        constraint fm_53_202301_rx_iso_day_check
            check (iso_day >= 0),
    date_release       timestamp with time zone,
    date_delivery      timestamp with time zone,
    date_visit         timestamp with time zone,
    month              smallint    not null
        constraint fm_53_202301_rx_month_check
            check (month >= 0),
    document_type      varchar(50),
    entity_id          integer     not null
        constraint fm_53_202301_rx_entity_id_d8660a11_fk_geo_entity_id
            references geo_entity
            deferrable initially deferred,
    area_id            varchar(32)
        constraint fm_53_202301_rx_area_id_70b37bf9_fk_med_cat_area_hex_hash
            references med_cat_area
            deferrable initially deferred,
    delivered_final_id varchar(32)
        constraint fm_53_202301_rx_delivered_final_id_331bd9fa_fk_med_cat_d
            references med_cat_delivered
            deferrable initially deferred,
    diagnosis_id       varchar(32)
        constraint fm_53_202301_rx_diagnosis_id_c4040beb_fk_med_cat_d
            references med_cat_diagnosis
            deferrable initially deferred,
    doctor_id          varchar(32)
        constraint fm_53_202301_rx_doctor_id_6f41a184_fk_med_cat_d
            references med_cat_doctor
            deferrable initially deferred,
    medical_unit_id    varchar(32)
        constraint fm_53_202301_rx_medical_unit_id_27e254eb_fk_med_c_liat_m
            references med_cat_medicalunit
            deferrable initially deferred,
    year               smallint
        constraint fm_53_202301_rx_year_check
            check (year >= 0),
    iso_delegation     smallint
        constraint fm_53_202301_rx_iso_delegation_check
            check (iso_delegation >= 0)
);

alter table fm_53_202301_rx
    owner to user_yeeko;

create table if not exists fm_53_202301_drug
(
    uuid              uuid    not null
        primary key,
    prescribed_amount smallint
        constraint fm_53_202301_drug_prescribed_amount_check
            check (prescribed_amount >= 0),
    delivered_amount  smallint
        constraint fm_53_202301_drug_delivered_amount_check
            check (delivered_amount >= 0),
    price             double precision,
    row_seq           integer
        constraint fm_53_202301_drug_row_seq_check
            check (row_seq >= 0),
    rx_id             uuid    not null
        constraint fm_53_202301_drug_rx_id_cdf044b3_fk_fm_p
            references fm_53_202301_rx
            deferrable initially deferred,
    sheet_file_id     integer not null,
    delivered_id      varchar(32)
        constraint fm_53_202301_drug_delivered_id_ff5c08d9_fk_med_cat_d
            references med_cat_delivered
            deferrable initially deferred,
    medicament_id     varchar(32)
        constraint fm_53_202301_drug_medicament_id_630af6c3_fk_med_cat_m
            references med_cat_medicament
            deferrable initially deferred,
    lap_sheet_id      integer not null,
    date_closed       timestamp with time zone,
...
alter table fm_53_202301_drug
    owner to user_yeeko;

create index if not exists fm_53_202301_drug_delivered_id_ff5c08d9
    on fm_53_202301_drug (delivered_id);

create index if not exists fm_53_202301_drug_medicament_id_630af6c3
    on fm_53_202301_drug (medicament_id);

create index if not exists fm_53_202301_drug_rx_id_cdf044b3
    on fm_53_202301_drug (rx_id);

create table if not exists fm_53_202301_missingrow
(
    uuid          uuid    not null
        constraint fm_53_202301_mr_pkey
            primary key,
    original_data jsonb,
    drug_id       uuid
        constraint fm_53_202301_mr_drug_id_746af424_fk_fm_drug_uuid
            references fm_53_202301_drug
            deferrable initially deferred,
    error         text,
    inserted      boolean,
    sheet_file_id integer not null
...
alter table fm_53_202301_missingrow
    owner to user_yeeko;

create table if not exists fm_53_202301_missingfield
(
    uuid           uuid                     not null
        constraint fm_53_202301_mf_pkey
            primary key,
    original_value text,
    final_value    text,
    other_values   jsonb,
    missing_row_id uuid                     not null
        constraint fm_53_202301_mf_missing_row_id_8903ee88_fk_fm_m
            references fm_53_202301_missingrow
            deferrable initially deferred,
    name_column_id integer                  not null
        constraint fm_53_202301_mf_name_column_id_d2bc6a65_fk_data_para
            references data_param_namecolumn
            deferrable initially deferred,
    error          text,
    inserted       boolean,
...
alter table fm_53_202301_missingfield
    owner to user_yeeko;

create index if not exists fm_53_202301_mf_missing_row_id_8903ee88
    on fm_53_202301_missingfield (missing_row_id);

create index if not exists fm_53_202301_mf_name_column_id_d2bc6a65
    on fm_53_202301_missingfield (name_column_id);

create index if not exists fm_53_202301_mr_drug_id_746af424
    on fm_53_202301_missingrow (drug_id);

create index if not exists fm_53_202301_rx_area_id_70b37bf9
    on fm_53_202301_rx (area_id);

create index if not exists fm_53_202301_rx_delivered_final_id_331bd9fa
    on fm_53_202301_rx (delivered_final_id);

create index if not exists fm_53_202301_rx_diagnosis_id_c4040beb
    on fm_53_202301_rx (diagnosis_id);

create index if not exists fm_53_202301_rx_doctor_id_6f41a184
    on fm_53_202301_rx (doctor_id);

create index if not exists fm_53_202301_rx_entity_id_d8660a11
    on fm_53_202301_rx (entity_id);

create index if not exists fm_53_202301_rx_medical_unit_id_27e254eb
    on fm_53_202301_rx (medical_unit_id);

create index if not exists same_name_index
    on fm_53_202301_rx (entity_id, iso_week, iso_year);

create table if not exists fm_55_201702_rx
(
    uuid_folio         uuid        not null
        primary key,
    folio_ocamis       varchar(64) not null,
    folio_document     varchar(46) not null,
    iso_year           smallint    not null
        constraint fm_55_201702_rx_iso_year_check
            check (iso_year >= 0),
    iso_week           smallint    not null
        constraint fm_55_201702_rx_iso_week_check
            check (iso_week >= 0),
    iso_day            smallint
        constraint fm_55_201702_rx_iso_day_check
            check (iso_day >= 0),
    date_release       timestamp with time zone,
    date_delivery      timestamp with time zone,
    date_visit         timestamp with time zone,
    month              smallint    not null
        constraint fm_55_201702_rx_month_check
            check (month >= 0),
    document_type      varchar(50),
    entity_id          integer     not null
        constraint fm_55_201702_rx_entity_id_d8660a11_fk_geo_entity_id
            references geo_entity
            deferrable initially deferred,
    area_id            varchar(32)
        constraint fm_55_201702_rx_area_id_70b37bf9_fk_med_cat_area_hex_hash
            references med_cat_area
            deferrable initially deferred,
    delivered_final_id varchar(32)
        constraint fm_55_201702_rx_delivered_final_id_331bd9fa_fk_med_cat_d
            references med_cat_delivered
            deferrable initially deferred,
    diagnosis_id       varchar(32)
        constraint fm_55_201702_rx_diagnosis_id_c4040beb_fk_med_cat_d
            references med_cat_diagnosis
            deferrable initially deferred,
    doctor_id          varchar(32)
        constraint fm_55_201702_rx_doctor_id_6f41a184_fk_med_cat_d
            references med_cat_doctor
            deferrable initially deferred,
    medical_unit_id    varchar(32)
        constraint fm_55_201702_rx_medical_unit_id_27e254eb_fk_med_c_liat_m
            references med_cat_medicalunit
            deferrable initially deferred,
    year               smallint
        constraint fm_55_201702_rx_year_check
            check (year >= 0),
    iso_delegation     smallint
        constraint fm_55_201702_rx_iso_delegation_check
            check (iso_delegation >= 0)
);

alter table fm_55_201702_rx
    owner to user_yeeko;

create table if not exists fm_55_201702_drug
(
    uuid              uuid    not null
        primary key,
    prescribed_amount smallint
        constraint fm_55_201702_drug_prescribed_amount_check
            check (prescribed_amount >= 0),
    delivered_amount  smallint
        constraint fm_55_201702_drug_delivered_amount_check
            check (delivered_amount >= 0),
    price             double precision,
    row_seq           integer
        constraint fm_55_201702_drug_row_seq_check
            check (row_seq >= 0),
    rx_id             uuid    not null
        constraint fm_55_201702_drug_rx_id_cdf044b3_fk_fm_p
            references fm_55_201702_rx
            deferrable initially deferred,
    sheet_file_id     integer not null,
    delivered_id      varchar(32)
        constraint fm_55_201702_drug_delivered_id_ff5c08d9_fk_med_cat_d
            references med_cat_delivered
            deferrable initially deferred,
    medicament_id     varchar(32)
        constraint fm_55_201702_drug_medicament_id_630af6c3_fk_med_cat_m
            references med_cat_medicament
            deferrable initially deferred,
    lap_sheet_id      integer not null,
    date_closed       timestamp with time zone,
...
alter table fm_55_201702_drug
    owner to user_yeeko;

create index if not exists fm_55_201702_drug_delivered_id_ff5c08d9
    on fm_55_201702_drug (delivered_id);

create index if not exists fm_55_201702_drug_medicament_id_630af6c3
    on fm_55_201702_drug (medicament_id);

create index if not exists fm_55_201702_drug_rx_id_cdf044b3
    on fm_55_201702_drug (rx_id);

create table if not exists fm_55_201702_missingrow
(
    uuid          uuid    not null
        constraint fm_55_201702_mr_pkey
            primary key,
    original_data jsonb,
    drug_id       uuid
        constraint fm_55_201702_mr_drug_id_746af424_fk_fm_drug_uuid
            references fm_55_201702_drug
            deferrable initially deferred,
    error         text,
    inserted      boolean,
    sheet_file_id integer not null
...
alter table fm_55_201702_missingrow
    owner to user_yeeko;

create table if not exists fm_55_201702_missingfield
(
    uuid           uuid                     not null
        constraint fm_55_201702_mf_pkey
            primary key,
    original_value text,
    final_value    text,
    other_values   jsonb,
    missing_row_id uuid                     not null
        constraint fm_55_201702_mf_missing_row_id_8903ee88_fk_fm_m
            references fm_55_201702_missingrow
            deferrable initially deferred,
    name_column_id integer                  not null
        constraint fm_55_201702_mf_name_column_id_d2bc6a65_fk_data_para
            references data_param_namecolumn
            deferrable initially deferred,
    error          text,
    inserted       boolean,
...
alter table fm_55_201702_missingfield
    owner to user_yeeko;

create index if not exists fm_55_201702_mf_missing_row_id_8903ee88
    on fm_55_201702_missingfield (missing_row_id);

create index if not exists fm_55_201702_mf_name_column_id_d2bc6a65
    on fm_55_201702_missingfield (name_column_id);

create index if not exists fm_55_201702_mr_drug_id_746af424
    on fm_55_201702_missingrow (drug_id);

create index if not exists fm_55_201702_rx_area_id_70b37bf9
    on fm_55_201702_rx (area_id);

create index if not exists fm_55_201702_rx_delivered_final_id_331bd9fa
    on fm_55_201702_rx (delivered_final_id);

create index if not exists fm_55_201702_rx_diagnosis_id_c4040beb
    on fm_55_201702_rx (diagnosis_id);

create index if not exists fm_55_201702_rx_doctor_id_6f41a184
    on fm_55_201702_rx (doctor_id);

create index if not exists fm_55_201702_rx_entity_id_d8660a11
    on fm_55_201702_rx (entity_id);

create index if not exists fm_55_201702_rx_medical_unit_id_27e254eb
    on fm_55_201702_rx (medical_unit_id);
...
(
    ...
    delivered_id     varchar(32) not null
        constraint mat_drug_totals_delivered_id_3ba71660_fk_med_cat_d
            references med_cat_delivered
            deferrable initially deferred,
    entity_week_id   integer     not null
        constraint mat_drug_totals_entity_week_id_84bd454f_fk_inai_entityweek_id
            references inai_entityweek
            deferrable initially deferred
);
...
create index if not exists mat_drug_totals_entity_week_id_84bd454f
    on mat_drug_totals (entity_week_id);
...
(
    ...
    delivered_id     varchar(32) not null
        constraint mat_drug_priority_delivered_id_f78d3b3b_fk_med_cat_d
            ...
            deferrable initially deferred,
    entity_week_id   integer     not null
        constraint mat_drug_priority_entity_week_id_69692165_fk_inai_entityweek_id
            references inai_entityweek
            deferrable initially deferred
);
...
create index if not exists mat_drug_priority_entity_week_id_69692165
    on mat_drug_priority (entity_week_id);
...
(
    ...
    delegation_id    integer  not null
        constraint mat_drug_extended_delegation_id_5dad7a7d_fk_geo_delegation_id
            ...
            deferrable initially deferred,
    entity_id        integer  not null
        constraint mat_drug_extended_entity_id_1d9e338c_fk_geo_entity_id
            references geo_entity
            deferrable initially deferred,
    presentation_id  integer  not null
...
create index if not exists mat_drug_extended_entity_id_1d9e338c
    on mat_drug_extended (entity_id);
...
(
    ...
    delivered_id     varchar(32)  not null
        constraint mat_drug_delivered_id_2b809653_fk_med_cat_delivered_hex_hash
            ...
            deferrable initially deferred,
    entity_week_id   integer      not null
        constraint mat_drug_entity_week_id_00716cdb_fk_inai_entityweek_id
            references inai_entityweek
            deferrable initially deferred
);
...
(
    ...
    area_id         varchar(32)
        constraint formula_complementrx_area_id_b0ff963e_fk_med_cat_area_hex_hash
            ...
            deferrable initially deferred,
    diagnosis_id    varchar(32)
        constraint formula_complementrx_diagnosis_id_b7888698_fk_med_cat_d
            references med_cat_diagnosis
            deferrable initially deferred,
    rx_id           uuid not null
...
SELECT formula_drug.entity_week_id,
       count(*) AS count
FROM formula_drug
GROUP BY formula_drug.entity_week_id;
...
GROUP BY week.iso_year, week.iso_week, week.entity_id, week.year, week.month, drug.delivered_id, drug.medicament_id;
...
GROUP BY rx.entity_id, rx.medical_unit_id, drug.entity_week_id, drug.delivered_id, drug.medicament_id;
...
SELECT rx.entity_id,
       ...
       count(*) AS count
FROM formula_rx rx
GROUP BY rx.entity_id, rx.delivered_final_id, rx.year, rx.month, rx.medical_unit_id;
