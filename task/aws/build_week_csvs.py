...
class BuildWeekAws:
    ...
    def __init__(self, event: dict, context):
        ...
        self.basic_fields = ["sheet_file_id", "folio_ocamis", "uuid_folio", "delivered_final_id"]
    ...
    def build_week_csvs(self):
        ...
        merge_rows = self.get_basic_data(merge_table_files)

        pos_rx_id = 1
        every_folios = {}

        pos_delivered = self.positions.get("delivered_final_id")
        for row in merge_rows:
            ...
            if current_folio["first_uuid"] != rx_id:
                current_drug[pos_rx_id] = current_folio["first_uuid"]
            self.buffers["drug"].writerow(current_drug)
            ...
            if len(current_folio["delivered"]) > 1:
                delivered, err = calculate_delivered_final(current_folio["delivered"])
                if delivered != current_folio["first_delivered"]:
                    current_folio["first_delivered"] = delivered
                    current_folio["rx_data"][pos_delivered] = delivered
